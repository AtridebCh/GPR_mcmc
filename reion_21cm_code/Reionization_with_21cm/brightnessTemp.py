...
import experimenting_numba_21_cm

from experimenting_numba_21_cm import running_code_with_21
...
class generating_21cm_signal:
	...
	def __init__(self,MCMC_H0, MCMC_ombh2, MCMC_omch2, f_X_2, f_R_3, f_alpha_3, redshift_edges,redshift_interpolate, pop2, pop3, f_R_2, f_X_3, f_alpha_2):
		...
		self.f_X_2=f_X_2
		self.f_X_3=f_X_3
		self.f_alpha_2=f_alpha_2
		self.f_alpha_3=f_alpha_3
		...
		self.pop3_sfrd=sp.interpolate.interp1d(self.redshift_array, pop3)	
	...
	def integrand_for_radio(self,z_prime,z):
		return self.f_R_2*self.pop2_sfrd(z_prime)/((1+z_prime)**2.1*self.Hubble(z_prime)*(mpc_to_m)**3)+self.f_R_3*self.pop3_sfrd(z_prime)/((1+z_prime)**2.1*self.Hubble(z_prime)*(mpc_to_m)**3)




	def integrand_for_lyalpha(self,z_prime):
		return self.f_alpha_2*lyalpha_pop2*self.pop2_sfrd(z_prime)/((1+z_prime)*365*24*3600*self.Hubble(z_prime))+self.f_alpha_3*lyalpha_pop3*self.pop3_sfrd(z_prime)/((1+z_prime)*365*24*3600*self.Hubble(z_prime))	
	...
	def func(self,z,T_k):
		dT_kdz=2.0*T_k/(1.0+z)-2.0/3.0*self.f_X_2*3.4*(10**33)*self.pop2_sfrd(z)/(boltzmann_constant*(1+z)*self.number_density_hydrogen*(mpc_to_m)**3*self.Hubble(z))-2.0/3.0*self.f_X_3*3.4*(10**33)*self.pop3_sfrd(z)/(boltzmann_constant*(1+z)*self.number_density_hydrogen*(mpc_to_m)**3*self.Hubble(z))
		return dT_kdz
	...
	def signal_generator(self):
		...
		x_alpha_1=self.x_alpha(self.redshift_array)
