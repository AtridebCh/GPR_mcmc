...
DEFAULT_GPR_MAPPING={'epsilon_a0':0,
					 'epsilon_a1':1,
					 'epsilon_a2':2,
					 'epsilon_a3':3,
					 'epsilon_a4':4,
...
class CoreModule(object):
	...
	"""
	def __init__(self, cosmodict, Astro_dict, GPR_mapping=DEFAULT_GPR_MAPPING):
		self.GPR_mapping = GPR_mapping
	...
	def __call__(self, ctx):
		...
		try:
			GPR_params = {}
			for k,v in self.GPR_mapping.items():
				GPR_params[k] = p[v]

			#build_interpolator pass it to Refshift_Evolution
			esc_PopII_redshift=np.array([4, 8, 12, 16, 20, 24]).reshape(-1, 1)
			esc_Pop_II_val=np.array([GPR_params['epsilon_a0'], GPR_params['epsilon_a1'], GPR_params['epsilon_a2'], GPR_params['epsilon_a3'], GPR_params['epsilon_a4'], GPR_params['epsilon_a5']]).reshape(-1, 1)
			...
			Redshift, QHII, tau, dnlldz, gamma_PI, QHII5point8=Redshift_Evolution(self.cosmodict['H0'], self.cosmodict['ombh2'], self.cosmodict['omch2'], self.cosmodict['sigma8'], self.cosmodict['ns'], self.Astro_dict['lambda_0'], 
 self.Astro_dict['esc_pop_III'], GPR_interpolator).quntity_for_MCMC()
