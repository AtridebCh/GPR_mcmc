...
class Likelihood(object):
	...
	def __call__(self, p, single_run=False):
		...
		if np.all(p>0.0) and np.all(p<1.0) :	
			model=self.Core(ctx)
			if model:
				likelihood, tau=self.ComputeLikelihood(ctx, single_run=single_run) #blobs feature is used and you have to use array for this
				blobs=np.array([tau])
				return likelihood, blobs
			else:
				return -np.inf, [1.0]
		else:
			return -np.inf, [1.0]  #if len(blobs)>1 then use [1.0, 1.0]
	...
	def setup(self, gammadatafile, Lymanlimitdatafile):
