...
class MCMCsampler():
	...
	def sample(self, InitPos, burninProb=None, burninRstate=None, datas=None):
		...
		nblobs=1 #Number of blobs one wants to use in emcee
		...
		for pos, prob, stat, blobs in self._sampler.sample(InitPos,iterations=self.sampleIterations):
			...
			if(counter%1==0):
				self.log("Iteration finished with total sample Number" + str(counter*self.nwalkers)) #Sample number from the MCMC after every step
